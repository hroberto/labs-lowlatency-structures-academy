...
> when does the modern STL already suffice, and when not?

## The failure question

> what does the decoder do with a truncated message?
...
`std::string` / `stringstream` → `string_view` + `from_chars` → a binary zero-copy decoder
...
| Topic | State |
|---|---|
| `01-string-vs-string-view` — `string` versus `string_view` | **not started** |
| `02-decoder-zero-copy` — zero-copy decoder | **not started** |
