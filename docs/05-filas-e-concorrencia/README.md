...
> quando o lock deixa de ser o gargalo e passa a ser a linha de cache?

## A pergunta de falha

> o que a fila faz quando enche — e quem descobre primeiro?
...
`std::queue` + `mutex` → SPSC → MPSC; `condition_variable` → *busy-poll* com `_mm_pause`
...
| Tópico | Estado |
|---|---|
| `01-mutex-queue` — fila com mutex | **não iniciado** |
| `02-spsc-lock-free` — SPSC *lock-free* | **não iniciado** |
| `03-mpsc` — MPSC | **não iniciado** |
| `04-wakeup-vs-busy-poll` — acordar contra *busy-poll* | **não iniciado** |
