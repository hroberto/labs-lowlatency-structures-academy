...
> em que cardinalidade a busca em array plano perde para a árvore?

## A pergunta de falha

> o que o *rehash* faz com a cauda?
...
`std::list` → `std::vector`; `std::map` → *flat sorted*; `std::unordered_map` → *open addressing*
...
| Tópico | Estado |
|---|---|
| `01-vector-vs-list` — `vector` contra `list` | **não iniciado** |
| `02-flat-map-vs-map` — *flat map* contra `map` | **não iniciado** |
| `03-open-addressing-vs-unordered` — *open addressing* contra `unordered_map` | **não iniciado** |
