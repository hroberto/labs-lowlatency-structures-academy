...
export CPP_ACADEMY_AMOSTRAS=3
export CPP_ACADEMY_RODADAS=20000

# --- 1. cauda suficiente: sem aviso, saida zero ---------------------------
if saida=$(CPP_ACADEMY_CAUDA=10000 "$BIN" 2>&1); then
    ok "o programa sai com zero quando a coleta e valida"
else
    erro "saiu com codigo diferente de zero:"; printf '%s\n' "$saida" | sed 's/^/    /' >&2
fi
for linha in "ler o relogio" "registrar amostra" "piso do par de leituras"; do
    case "$saida" in
        *"$linha"*) ;;
...
case "$saida" in
    *"NAO e publicavel"*) erro "avisou percentil sem sustentacao com 10 000 amostras" ;;
...
case "$saida" in
    *"autorizam"*"LOTE"*) ok "a saida diz o que os numeros autorizam e aponta a medicao em lote" ;;
    *) erro "a saida nao diz o que os numeros autorizam" ;;
esac

# --- 2. braco de controle: cauda curta TEM de avisar ----------------------
if saida_curta=$(CPP_ACADEMY_CAUDA=500 "$BIN" 2>&1); then
    case "$saida_curta" in
        *"NAO e publicavel"*)
...
for modo in "" "--csv"; do
    rotulo=${modo:-texto}
    if CPP_ACADEMY_CAUDA=2000 "$BIN" --braco-de-controle $modo >/dev/null 2>&1; then
...
msg=$(CPP_ACADEMY_CAUDA=2000 "$BIN" --braco-de-controle 2>&1 || true)
case "$msg" in
    *"removido pelo otimizador"*) ok "a falha explica que mediana zero e laco removido" ;;
    *) erro "a mensagem de falha nao explica a causa da mediana zero" ;;
esac

# --- 4. modo CSV: e o que a campanha arquiva -----------------------------
if csv=$(CPP_ACADEMY_CAUDA=2000 "$BIN" --csv 2>&1); then
    cabecalho=$(printf '%s\n' "$csv" | head -1)
    [ "$cabecalho" = "braco,metrica,valor,unidade" ] \
    ...
    for campo in per_op_floor,p50 per_op_floor,p999 per_op_floor,highest_supported \
                 per_op_floor,discarded parametros,amostras parametros,clock_period; do
        case "$csv" in
            *"$campo"*) ;;
